...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import vmtouch_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("28f2fcfb-5f6a-5f77-b781-4b42819ab36c"))
platforms = [Pkg.Artifacts.unpack_platform(e, "vmtouch", artifacts_toml) for e in artifacts["vmtouch"]]
...
if best_platform === nothing
    @debug("Unable to load vmtouch; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module vmtouch_jll
